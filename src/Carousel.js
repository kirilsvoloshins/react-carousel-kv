...
export function Carousel(props) {
    ...
    useEffect(() => {
        ...
        changeSlideContent(currentlySelectedSlideId); /* return slide container to the initial state. */
    }, [currentlySelectedSlideId]);

    /* SMALL HELPER FUNCTIONS */
    const getCleanSlideData = slideId => `<div>${typeof slides[slideId] === "undefined" ? "" : slides[slideId]}</div>`; /* will not show "undefined" instead of non-existing slides */
    const getUserSwipeDirection = () => ({ isUserSwipingRight: newMarginLeft < -100, isUserSwipingLeft: newMarginLeft >= -100 });
    const wasSwipeTooFast = ({ timeStamp }) => {
        const MIN_SWIPE_DURATION_MS = 100;
        const swipeDuration = timeStamp - swipeStartData.current.timeStamp;
    ...
    const isThereNoSlideThere = (isUserSwipingRight) => {
    ...
    const getSlidesDataHTML = slideId => [getCleanSlideData(slideId - 1), getCleanSlideData(slideId), getCleanSlideData(slideId + 1)].join("");
    const getSlidesDataHTML_duplicate12 = slideId => [getCleanSlideData(slideId), getCleanSlideData(slideId), getCleanSlideData(slideId + 1)].join(""); /* animating slides the "easy" way turned out to be quite complex since I have chosen to store simultaneously only three slides and animations are done by changing marginLeft. The way it works is there are three slides on screen simultaneously. The user sees only the middle one - previous and next ones are hidden. Once the user swipes to the next slide, the previous and current are hidden. Since there are only three slides and I kinda need to reset the margin for selected slide to be the middle one (so the user can continue switching between slides), I duplicate the content of the slide being shown to the middle slide and change marginLeft back for the middle slide to be shown. Then I update the content of all slides to store newPrevious, newCurrent and newNext slide actual content.*/
    const getSlidesDataHTML_duplicate23 = slideId => [getCleanSlideData(slideId), getCleanSlideData(slideId + 1), getCleanSlideData(slideId + 1)].join("");
    const changeSlideContent = slideId => contentContainer.current.innerHTML = getSlidesDataHTML(slideId);
    const changeSlideContentDuplicateVisibleSlideToNextOne = slideId => contentContainer.current.innerHTML = getSlidesDataHTML_duplicate12(slideId);
    const changeSlideContentDuplicateVisibleSlideToPreviousOne = slideId => contentContainer.current.innerHTML = getSlidesDataHTML_duplicate23(slideId);
    const returnClosestPossibleSlideIndex = passedValue => {
    ...
    const returnRecommendedSlideIndex = humanValue => {
        if (isNaN(humanValue)) return firstSlideId;
        const properSlideIndex = humanValue - 1;
        return returnClosestPossibleSlideIndex(properSlideIndex);
    ...
    }; /* opening the previous slide is actually a for loop with setTimeouts to "animate". */
    ...
    }; /* opening the next slide is actually a for loop with setTimeouts to "animate". */
    async function animateWhatsLeftToGoToPreviousSlide() {
        await animateLeft();
        const newCurrentlySelectedSlideId = currentlySelectedSlideId - 1;
        changeSlideContentDuplicateVisibleSlideToNextOne(newCurrentlySelectedSlideId);
        setMarginLeft(-100);
        selectSlideById(newCurrentlySelectedSlideId);
    }; /* this fn is kinda universal since it can be used for switching slides both from buttons and swipes. */
    async function animateWhatsLeftToGoToNextSlide() {
        await animateRight();
        const newCurrentlySelectedSlideId = currentlySelectedSlideId + 1;
        changeSlideContentDuplicateVisibleSlideToPreviousOne(newCurrentlySelectedSlideId);
        setMarginLeft(-100);
        selectSlideById(newCurrentlySelectedSlideId);
    }; /* this fn is kinda universal since it can be used for switching slides both from buttons and swipes. */
    ...
    function handleSlideNumberManualInput() {
        ...
        const properSlideIndex = returnRecommendedSlideIndex(valueBeingInput);
    ...
    function handleSwipeEnd(e) {
        ...
        if (wasSwipeTooFast(e) || wasSwipeTooShort(e) || isThereNoSlideThere(isUserSwipingRight)) {
            setMarginLeft(-100);
